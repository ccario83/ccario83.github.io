...
function Line(color, id, polars)
{
    this.pts = [];
    this.color = color;
    this.segment = .35 + Math.random() * .15;
...
var stage = new tools.Stage(150, 150);
var out = stage.out;

var temp = new tools.Stage(150, 150, false);

//stage.autoSize(onResize);
stage.resize(200,100);
var nCoords = 200;
...
var particles = [];
var nParticles = 15;
var radiusBoundaryForce = 0.000002;
...
{
    var p = new Particle();
    p.size = 10 * Math.pow(i/nParticles, 4) + 5;
    var a = Math.random() * 1.2 * Math.PI;
...
function updateParticles()
{
    ...
    for(var i = 0; i < nMicroParticles; i++)
    {
      ...
      p.vx -= 0.00002 * p.x;
      p.vy -= 0.00002 * p.y;
      p.vx = 0.59 * p.vx + 0.1 * (Math.random() * 2 - 1);
      p.vy = 0.59 * p.vy + 0.1 * (Math.random() * 2 - 1);
      p.x += p.vx;
      p.y += p.vy;
    }
}
